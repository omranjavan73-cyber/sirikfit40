...
import { calculateLandedPrice } from '../../services/pricingService';
...
export const ProductManagementAdmin: React.FC<ProductManagementAdminProps> = ({
  ...
  const handleScrapeMain = async () => {
    ...
    try {
      ...
      if (data && (data.title || data.titleEn || data.priceAED || data.priceAed || data.price)) {
        const scraped = data;
        ...
        const resolvedStore = isIherbLink ? 'iHerb' : (scraped.storeName || originInfo?.storeName || 'iHerb');
