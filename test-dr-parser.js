async function testDrNutritionParser() {
  const url = 'https://drnutrition.com/en-ae/applied-nutrition-bcaa-amino-hydrate-32-serving-green-apple';
  const jinaUrl = 'https://r.jina.ai/' + url;
  
  const res = await fetch(jinaUrl);
  const md = await res.text();

  // Helper to extract clean image url from Dr Nutrition Next.js image wrapper or raw url
  ...
  // 2. Brand extraction
  let brand = '';
  const brandMatch = md.match(/By\s+([A-Za-z0-9\s&.-]+)/i);
  if (brandMatch) {
  ...
  // Find option block between main title and 'Delivery' / 'ADD TO CART' / 'Ship to your address'
  ...
  while ((secMatch = sectionRegex.exec(optionsContent)) !== null) {
    ...
    if (['Description', 'How to Use', 'Health Notes', 'Product Details', 'Features', 'Nutrition Facts', 'Service Guarantees', 'Resources', 'Quick Links'].some(x => groupHeading.toLowerCase().includes(x.toLowerCase()))) {
      continue;
    }

    const groupBody = secMatch[2];
    const itemsMap = new Map(); // Name -> { name, image, inStock }

    // Each option item is formatted as: `![Image X: Label](image_url)OptionText` or lines with text
    ...
    for (const line of lines) {
      ...
      if (imgItemMatch) {
        ...
        // If itemText is empty, use the alt text
  ...
  // 7. Extract Real Variants Matrix
  const variants = [];
  ...
  console.log('Brand:', finalResult.brand);
  console.log('Title:', finalResult.title);
  console.log('Price:', finalResult.price, 'AED | Original:', finalResult.originalPrice, 'AED | Discount:', finalResult.discountPercent + '%');
  console.log('Option Groups Count:', finalResult.options.length);
  console.log('Option Groups:', JSON.stringify(finalResult.options, null, 2));
  console.log('Selected Variant:', JSON.stringify(finalResult.selectedVariant, null, 2));
  console.log('Variants Count:', finalResult.variants.length);
  console.log('Sample Variant (first 2):', JSON.stringify(finalResult.variants.slice(0, 2), null, 2));
}

testDrNutritionParser();
